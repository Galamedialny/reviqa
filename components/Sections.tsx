...
export function PageHero({ eyebrow, title, text, image }: { eyebrow: string; title: string; text: React.ReactNode; image?: string }) {
  return (
...
export function ServiceGrid({ limit }: { limit?: number }) {
  return <div className="service-grid">{services.slice(0, limit ?? services.length).map((service, i) => (
    <article className="service-card" key={service.slug}>
      <div className="service-image"><img src={asset(service.image)} alt={`Tematická ilustrácia terapie – ${service.title}`} /><span>Tematická ilustrácia</span></div>
