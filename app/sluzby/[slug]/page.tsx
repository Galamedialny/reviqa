...
export default async function ServicePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><StructuredData data={structuredData} /><PageHero eyebrow={service.eyebrow} title={service.title} text={service.intro} image={service.image} />
    ...
    {service.slug === "chiromanualne-terapie"
      ? <section className="section shell"><SectionHeading eyebrow="Možnosti starostlivosti" title="Postup volíme podľa konkrétneho klienta" /><div className="care-copy"><p>Nezameriavame sa iba na miesto bolesti. Cielene pracujeme s funkčnými blokádami chrbtice, kĺbov, svalov, fascií a nervových dráh, ktoré môžu ovplyvňovať prirodzenú funkciu organizmu.</p><p>Pomocou špecializovaných manuálnych techník vytvárame optimálne podmienky pre prirodzenú regeneráciu organizmu, obnovu funkčných schopností a dlhodobé udržanie zdravia. Každá terapia je individuálne prispôsobená aktuálnemu funkčnému stavu, potrebám a cieľom klienta.</p></div></section>
    ...
    <section className="section dark-panel"><div className={`shell service-content${service.details.length === 0 ? " service-content-single" : ""}`}><div><SectionHeading eyebrow="Pre koho môže byť vhodná" title="Najskôr posúdime súvislosti" /><ul className="gold-list service-audience">{service.suitableFor.map((item) => <li key={item}>{item}</li>)}</ul></div>{service.details.length > 0 && <div className="content-blocks">{service.details.map(([title, text]) => <article key={title}><h3>{title}</h3><p>{text}</p></article>)}</div>}</div></section>
